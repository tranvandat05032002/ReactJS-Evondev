import axios from "axios";
import React from "react";
// import { debounce } from "lodash";
import { useState } from "react";
import { useEffect } from "react";

import useDebounce from "../hooks/useDebounce";
import LoadingSkeleton from "../loading/LoadingSkeleton";
//https://api.themoviedb.org/3/movie/550?api_key=2537abce0574afa219f72b4d7aacde04
//https://api.themoviedb.org/3/search/movie?api_key=2537abce0574afa219f72b4d7aacde04&query=''
const MovieApp = () => {
  const [movies, setMovies] = useState([]);
  const [query, setQuery] = useState("");
  const [loading, setLoading] = useState(true);
  const queryDebounce = useDebounce(query, 1000);
  useEffect(() => {
    setLoading(true);
    const fetchData = async () => {
      const response = await axios.get(
        `https://api.themoviedb.org/3/search/movie?api_key=2537abce0574afa219f72b4d7aacde04&query='${queryDebounce}'`
      );
      if (response.data.results) {
        setMovies(response.data.results);
        setLoading(false);
      }
    };
    fetchData();
  }, [queryDebounce]);
  return (
    <div className="p-7">
      <div className="w-full max-w-[500px] mx-auto">
        <input
          type="text"
          className="w-full px-5 py-3 rounded-lg border-[1px] border-[#7D6AFF] shadow-[0px_0px_0px_3px_rgba(125,_106,_255,_0.2)] focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1"
          placeholder="Search Movie"
          onChange={(e) => setQuery(e.target.value)}
        />
      </div>
      {loading && (
        <div className="grid grid-cols-3 gap-10 mt-[100px]">
          <MovieItemSkeleton></MovieItemSkeleton>
          <MovieItemSkeleton></MovieItemSkeleton>
          <MovieItemSkeleton></MovieItemSkeleton>
        </div>
      )}
      <div className="grid grid-cols-3 gap-10 mt-[100px]">
        {!loading &&
          movies.length > 0 &&
          movies.map((item, index) => (
            <MovieItem key={item.id} data={item}></MovieItem>
          ))}
      </div>
    </div>
  );
};

//Skeleton
const MovieItemSkeleton = () => {
  return (
    <div className="p-[10px] rounded-2xl bg-white flex flex-col">
      <div className="h-[297px] ">
        <LoadingSkeleton
          width="100%"
          height="100%"
          radius="16px"
        ></LoadingSkeleton>
      </div>
      <div className="p-10 flex flex-1 flex-col">
        <h3 className="text-[#333] text-base font-semibold mb-4">
          <LoadingSkeleton height="20px"></LoadingSkeleton>
        </h3>
        <span className="text-sm font-normal text-[#999] mb-3">
          <LoadingSkeleton height="10px"></LoadingSkeleton>
          <div className="h-1"></div>
          <LoadingSkeleton height="10px"></LoadingSkeleton>
          <div className="h-1"></div>
          <LoadingSkeleton height="10px"></LoadingSkeleton>
        </span>
        <div className="flex items-center gap-x-[9px] mt-auto">
          {/* <svg
            width="16"
            height="15"
            viewBox="0 0 16 15"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M7.66713 1.02447C7.7719 0.702008 8.2281 0.702009 8.33287 1.02447L9.71753 5.28602C9.76439 5.43023 9.89877 5.52786 10.0504 5.52786H14.5313C14.8703 5.52786 15.0113 5.96173 14.737 6.16102L11.1119 8.7948C10.9892 8.88393 10.9379 9.04191 10.9847 9.18612L12.3694 13.4477C12.4742 13.7701 12.1051 14.0383 11.8308 13.839L8.20572 11.2052C8.08305 11.1161 7.91695 11.1161 7.79428 11.2052L4.16918 13.839C3.89488 14.0383 3.52581 13.7701 3.63059 13.4477L5.01525 9.18612C5.06211 9.04191 5.01078 8.88393 4.88811 8.7948L1.26301 6.16102C0.988711 5.96173 1.12968 5.52786 1.46874 5.52786H5.9496C6.10123 5.52786 6.23561 5.43023 6.28247 5.28602L7.66713 1.02447Z"
              stroke="#FFB86C"
              strokeWidth="1.5"
            />
          </svg> */}
          <LoadingSkeleton height="10px" width="16px"></LoadingSkeleton>

          <span className="font-semibold text-sm text-[#333]">
            {/* vote */}
            <LoadingSkeleton height="10px" width="46px"></LoadingSkeleton>
          </span>
        </div>
      </div>
    </div>
  );
};

//Movie Item
const MovieItem = ({ data }) => {
  return (
    <div className="p-[10px] rounded-2xl shadow-sm bg-white flex flex-col">
      <div className="h-[297px]">
        <img
          src={`https://image.tmdb.org/t/p/original/${
            data.poster_path !== null
              ? data.poster_path
              : "wwemzKWzjKYJFfCeiB57q3r4Bcm.png"
          }`}
          alt=""
          className="w-full h-full rounded-lg object-cover"
        />
      </div>
      <div className="p-10 flex flex-1 flex-col">
        <h3 className="text-[#333] text-base font-semibold mb-4">
          {data.title}
        </h3>
        <span className="text-sm font-normal text-[#999]">{data.overview}</span>
        <div className="flex items-center gap-x-[9px] mt-auto">
          <svg
            width="16"
            height="15"
            viewBox="0 0 16 15"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M7.66713 1.02447C7.7719 0.702008 8.2281 0.702009 8.33287 1.02447L9.71753 5.28602C9.76439 5.43023 9.89877 5.52786 10.0504 5.52786H14.5313C14.8703 5.52786 15.0113 5.96173 14.737 6.16102L11.1119 8.7948C10.9892 8.88393 10.9379 9.04191 10.9847 9.18612L12.3694 13.4477C12.4742 13.7701 12.1051 14.0383 11.8308 13.839L8.20572 11.2052C8.08305 11.1161 7.91695 11.1161 7.79428 11.2052L4.16918 13.839C3.89488 14.0383 3.52581 13.7701 3.63059 13.4477L5.01525 9.18612C5.06211 9.04191 5.01078 8.88393 4.88811 8.7948L1.26301 6.16102C0.988711 5.96173 1.12968 5.52786 1.46874 5.52786H5.9496C6.10123 5.52786 6.23561 5.43023 6.28247 5.28602L7.66713 1.02447Z"
              stroke="#FFB86C"
              strokeWidth="1.5"
            />
          </svg>

          <span className="font-semibold text-sm text-[#333]">
            {data.vote_average}
          </span>
        </div>
      </div>
    </div>
  );
};

export default MovieApp;
